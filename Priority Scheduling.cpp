...
void disp(int n){
    ...
    for(i=0;i<n;i++){
        ...
        sort(proc+i,proc+j,prioritySort);
        tArray[i]=ttime;
    ...
    cout<<"\n\n";
    cout<<"Processes  Arrival Time\t Priority\t Burst Time \t Turn Around \t Waiting Time\n";
 
    for (i=0; i<n; i++){
        cout << proc[i].ccode << "\t\t"; //Process
        cout << proc[i].arrival_time << "\t\t"; //Arrival
        cout << proc[i].priority << "\t\t"; //Arrival
